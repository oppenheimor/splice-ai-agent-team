...
const checks = [
  ...
  includesAll("Prompt 引用空洞报告防线", ["DEEP_DIAGNOSIS_ANTI_EMPTY_REPORT_RULES", "DEEP_DIAGNOSIS_BOTTLENECK_TYPES", "DEEP_DIAGNOSIS_DELIVERABLE_ASSET_TYPES"]),
  includesAll("Prompt 强制外部分析对照", ["DEEP_DIAGNOSIS_EXTERNAL_RESEARCH_SECTIONS", "行业分析报告", "诊断报告", "用户现场对照", "外部模式", "用户现实", "未验证缺口"]),
  includesAll("Prompt 强制外部搜索节制", ["默认每个外部研究节点只调用 1 次 webSearch", "才允许补搜 1 次", "证据缺口", "不要为了行业趋势、案例、工具、竞品各查一次"]),
  sourceIncludesAll("基础 Prompt 默认一次综合搜索", requirementsPromptSource, ["默认只做一次综合搜索", "才允许补搜 1 次", "coverage 明显不足", "不要为了行业趋势、案例、工具选型各查一次"]),
  includesAll("Prompt 强制瓶颈诊断", ["第一瓶颈", "证据来源", "暂不建议", "真实样本", "可用资产"]),
  includesAll("Prompt 强制验证闭环", ["成功阈值", "失败阈值", "推翻当前结论"]),
  includesAll("Prompt 强制范围与横扫", ["诊断范围等级", "报告级别", "横向扫描", "至少 3 个候选"]),
  includesAll("Prompt 强制横扫矩阵", ["DEEP_DIAGNOSIS_HORIZONTAL_SCAN_MATRIX_COLUMNS", "评分分解矩阵", "候选环节", "第一瓶颈匹配度", "为什么不是其它环节"]),
  includesAll("Prompt 强制入口路由", ["入口范围路由器", "先全局盘点", "聚焦一个问题", "还说不清楚", "mode: single", "allowOther: false"]),
  includesAll("Prompt 强制多问题横扫", ["mode: multiple", "minSelections: 1", "不要设置 maxSelections", "allowOther: true", "可以只选 1 个，也可以全选", "优先深挖 1-2 个", "业务横向扫描 + 单流程深度诊断"]),
  includesAll("Prompt 多选文案去黑话", ["请选择你想一起评估的问题", "我会先帮你一起比较"]),
  includesAll("Prompt 强制交互契约", ["交互契约", "DEEP_DIAGNOSIS_INTERACTION_CONTRACTS", "不要让正文提出一个开放式问题，同时又调用一个只能选择的 UI 工具"]),
  includesAll("Prompt 强制全局盘点前置上下文", ["业务类型、用户角色、当前阶段", "只用纯文本追问", "不要调用 askUserChoice", "只有业务类型、用户角色、当前阶段已经明确之后"]),
  ...
  includesAll("Prompt 强制事实确认", ["已确认事实", "待确认事实", "未覆盖区域", "用户跳过关键事实确认", "禁止同一轮一边列出非空「待确认事实」", "如果你列出的「未覆盖区域」非空"]),
  includesAll("Prompt 强制事实确认三选一", ["DEEP_DIAGNOSIS_FACT_CONFIRMATION_OPTIONS", "补齐关键事实", "跳过并降级出假设简报", "继续诊断未覆盖模块"]),
  includesAll("Prompt 强制完整报告就绪门禁", ["完整报告就绪门禁", "最小事实包", "真实样本或明确缺失", "反选原因", "不要出现「出完整方案」选项"]),
  includesAll("Prompt 强制反选追问", ["横向扫描推荐 A", "用户选择 B", "为什么你更想看 B", "用户回答前，不允许出完整报告"]),
  includesAll("Prompt 强制证据链和反例", ["搜索覆盖范围", "原始引用链", "反例", "工具失败状态"]),
  includesAll("Prompt 强制证据绑定", ["DEEP_DIAGNOSIS_EVIDENCE_BINDING_COLUMNS", "证据绑定表", "关键结论", "未验证缺口"]),
  includesAll("Prompt 强制质量门禁", ["报告质量 Gate", "110 分制", "低于 90 分", "完整诊断书"]),
  ...
  includesAll("Prompt 强制接地气", ["接地气程度", "用户当前现场", "当前基线", "低成本", "7 天内", "现在不用做什么"]),
  includesAll("Prompt 强制自动化边界和任务化", ["可完全自动化", "AI 辅助但必须人审", "禁止自动化", "本周任务", "验收标准"]),
  includesAll("Prompt 强制升级路径", ["升级路径与定制适配度", "自助工具版", "工作流模板版", "系统 / Agent 定制版", "DEEP_DIAGNOSIS_CUSTOMIZATION_TRIGGER_SIGNALS"]),
  includesAll("Prompt 服从决策策略层", ["Deep Diagnosis 决策策略层", "运行时裁判", "优先级高于本 prompt", "策略层允许"]),
  includesAll("Prompt 强制定制 readiness", ["软触发信号", "硬触发信号", "阻塞信号", "定制 readiness 规则", "定制前置条件"]),
  ...
  includesAll("完整方案确认流程", ["askUserChoice", "generate_report", "continue_diagnosis", "出完整方案", "再诊断一轮", "当前不要承诺生成可访问 HTML 链接"]),
  ...
  sourceIncludesAll("结构化运行状态聚合产品层", runtimeStateSource, ["buildDeepDiagnosisFactCard", "buildDeepDiagnosisEvidenceView", "evaluateDeepDiagnosisReportReadiness", "buildDeepDiagnosisRuntimeQualityGate"]),
  ...
  sourceIncludesAll("上下文管理注入产品化骨架", contextManagerSource, ["formatDeepDiagnosisRuntimeStateForPrompt", "formatDeepDiagnosisFactCardForPrompt", "formatDeepDiagnosisEvidenceViewForPrompt", "formatDeepDiagnosisRuntimeQualityGateForPrompt"]),
  ...
  sourceIncludesAll("HTML 发布 TODO 边界", protocolSource, ["TODO: publishHtmlReport", "TODO: deep-diagnosis 独立会话", "TODO: 深度诊断价格 / 价值边界"]),
